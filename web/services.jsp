<jsp:include page="/includes/header.jsp" />


<!-- start the middle column -->
<div class="wrapper">
    <div class="header header-filter"  id="header-img-services">
        <div class="container">
            <div class="row" id="headerRow">
                <div class="col-md-12 text-center" id="header_text">
                    <h1 class="title" id="faq">Our Services</h1>
                    <br />

                </div>
            </div>
        </div>
    </div>	
</div>
<div class="main main-raised" id="main-raised">
    <div class="container">
        <div class="profile" id="faq-icon" >
            <div class="avatar" >
                <img src="/images/service-icon.png" alt="Circle Image" class="img-circle img-responsive img-raised">
                <br>
            </div>
        </div>
        <div class="row">
            <div class="col-md-4">
                <h2 class="text-center">Design Work</h2>
                <h4 class="text-center">
                    The fees associated with designing new and improved
                    bathroom and kitchen remodels include a minimum
                    charge of <strong>$500</strong> per kitchen which also includes the
                    on-site visits needed to take measurements create precise
                    sketches. We charge a baseline hourly rate of <strong>$100</strong>.
                </h4>
            </div>
            <div class="col-md-4">
                <h2 class="text-center">Cabinetry Installation</h2>
                <h4 class="text-center">
                    The minimum charge for the installation of cabinets for a
                    kitchen is <strong>$750</strong>. The hourly rate we charge
                    for this service is <strong>$75</strong>.
                </h4>
            </div>
            <div class="col-md-4">
                <h2 class="text-center">Cabinetry Re-facing</h2>
                <h4 class="text-center">
                    Taking on a similar process in comparison to our
                    cabinetry installation service, we also charge a minimum
                    of <strong>$750</strong> per kitchen along with an hourly 
                    labor rate of <strong>$75</strong>
                </h4>
            </div>
        </div>
        <div class="row">
            <div class="col-md-4">
                <h2 class="text-center">Plumbing Work</h2>
                <h4 class="text-center">
                    The plumbing services we offer generally include
                    bathrooms, kitchens, laundry rooms, or any exterior
                    plumbing our customers may request. We charge a minimum 
                    fee of <strong>$500</strong> per kitchen and an hourly 
                    labor rate of <strong>$85</strong>.
                </h4>
            </div>
            <div class="col-md-4">
                <h2 class="text-center">Electrical Work</h2>
                <h4 class="text-center">
                    Any and all electrical work that a customer's home
                    may require is similar to the fees of that of plumbing
                    work which is a minimum charge of <strong>$500</strong>
                    per kitchen and <strong>$85</strong> per hour for other
                    electrical work that the customer requests. 
                </h4>
            </div>
            <div class="col-md-4">
                <h2 class="text-center">Painting / Wallpaper Work</h2>
                <h4 class='text-center'>
                    We contract to an outside painting contractor 
                    <strong>(AZ Design)</strong>. They charge their customers
                    a minimum of <strong>$500</strong> to paint a kitchen and
                    <strong>$40</strong> per hour to hang wallpaper. The customer
                    will meet with the sub-contractor for more details.
                </h4>
            </div>
        </div>
        <div class="row">
            <div class="col-md-4 col-md-offset-2">
                <h2 class="text-center">General Construction Work</h2>
                <h4 class="text-center">
                    The services we offer that is in the category
                    of general construction work includes moving walls,
                    sheetrock, framing, windows, etc. Some of this work may
                    be contracted out if needed, but normal charges are
                    <strong>$60</strong> per hour for labor.
                </h4>
            </div>
            <div class="col-md-4">
                <h2 class="text-center">Deliveries</h2>
                <h4 class="text-center">
                    Our delivery fees are as follows: <strong>$50</strong> 
                    minimum charge for the 1st piece and then 
                    <strong>$25</strong> for each extra piece. Large items will
                    incur a <strong>$100</strong> surcharge for delivery. Any
                    installation that is required by the customer will be charged
                    at the standard hourly rate of <strong>$85</strong> per hour.
                </h4>
            </div>
            
        </div>
    </div>
</div>
<!-- end the middle column -->


<jsp:include page="/includes/footer.jsp" />


